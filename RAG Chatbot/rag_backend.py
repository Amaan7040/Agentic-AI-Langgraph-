...
def chat_node(state: ChatState, config=None):
    ...
    # Create system message with proper context
    system_content = "You are a helpful assistant with access to various tools. "
    if thread_id and thread_has_document(thread_id):
        system_content += f"You can use the RAG tool to answer questions about uploaded documents (thread_id: {thread_id}). "
    system_content += "You can also use other tools like web search, stock information, weather data, astronomical data, and URL analysis as needed."

    system_message = SystemMessage(content=system_content)

    # Properly format messages: system message followed by conversation history
    ...
    try:
        ...
        error_message = AIMessage(content=f"I encountered an error: {str(e)}. Please try again with a different query.")
        return {"messages": [error_message]}


# Define all tools properly
tools = [
    search_info,
    stock_details,
    url_metadata,
