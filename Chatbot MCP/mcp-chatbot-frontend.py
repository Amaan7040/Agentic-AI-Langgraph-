...
sys.path.append(r"C:\Users\khana\Downloads\Langgraph Agentic AI\Chatbot MCP")

# Import from MCP backend - corrected import name
from mcp_chatbot_backend_client import get_chatbot, retrieve_chats, model
load_dotenv()

# Global variables for chatbot and checkpointer
...
def initialize_chatbot():
    """Initialize the chatbot backend with MCP tools"""
    global _chatbot_instance, _checkpointer_instance
    
    if _chatbot_instance is None:
        loop = get_or_create_event_loop()
        _chatbot_instance, _checkpointer_instance = loop.run_until_complete(get_chatbot())
    
    return _chatbot_instance, _checkpointer_instance
...
with st.sidebar.expander("🛠️ Available MCP Tools", expanded=False):
    st.markdown("""
    **Connected Tools:**
    - 🔍 **Web Search** - DuckDuckGo search
    - 📈 **Stock Details** - Real-time stock prices
    - 🌐 **URL Metadata** - Webpage content extraction
    - 🌤️ **Current Weather** - Live weather data
    - 🌙 **Astronomical Data** - Sun/moon times
    - 📅 **Weather Forecast** - Multi-day forecasts
    """)
...
if real_chats:
    for thread_id in real_chats[::-1]:  # Show most recent first
        ...
        if st.sidebar.button(button_label, key=f"chat_{thread_id}", use_container_width=True):
            st.session_state['thread_id'] = thread_id
            messages = load_conversation(thread_id)
...
# Main chat interface
st.title("💬 ChatterBot with MCP")
st.caption("🚀 AI assistant powered by LangGraph and Model Context Protocol")
...
if user_input:
    ...
    # Stream AI response
    # This enables langsmith to trace chat turns and show it in its UI
    CONFIG = {
        'configurable': {'thread_id': st.session_state["thread_id"]},
        "metadata": {"thread_id": st.session_state["thread_id"]},
        "run_name": "mcp_chatbot_run"
    }
